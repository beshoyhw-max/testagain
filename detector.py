...
class PhoneDetector:
    def __init__(self, model_path='yolo26n.pt', pose_model_path='yolo26n-pose.pt', 
        ...
        self.face_rec_error_limit = 10  # Limit error logging to avoid spam
        ...
        # FIXED: Person identification with dynamic caching based on confidence
        ...
        self.identity_min_confidence = 0.40  # Minimum confidence to accept
        
        # Dynamic check intervals based on confidence level
        self.check_interval_high_conf = 3.0    # Confidence > 0.5: recheck every 3s
        self.check_interval_medium_conf = 2.0  # Confidence 0.4-0.5: recheck every 2s  
        self.check_interval_unknown = 0.5      # Unknown/low: recheck every 0.5s
        ...
        print(f"     • Face recognition: Dynamic intervals (0.5s-3s based on confidence)")
        ...
        if frame_count % skip_frames == 0:
            ...
            if self.face_recognizer:
                ...
                for p_box in person_boxes:
                    ...
                    # Dynamic check interval based on current confidence
                    current_confidence = self.identity_confidences.get(track_id, 0.0)
                    if current_confidence > 0.5:
                        check_interval = self.check_interval_high_conf
                    elif current_confidence >= self.identity_min_confidence:
                        check_interval = self.check_interval_medium_conf
                    else:
                        check_interval = self.check_interval_unknown
                    ...
                    if should_check:
                        try:
                            ...
                            # Log errors with limit to avoid spam
                            if self.face_rec_debug_counter < self.face_rec_error_limit:
                                print(f"  ⚠ Face rec error for ID {track_id}: {type(e).__name__}: {e}")
