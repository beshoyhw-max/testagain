...
# Try importing FAISS for fast similarity search
try:
    import faiss
    FAISS_AVAILABLE = True
except ImportError:
    FAISS_AVAILABLE = False
    print("[WARNING] FAISS not installed. Using linear search. Install with: pip install faiss-cpu")
...
class FaceRecognizer:
    ...
    def __init__(self, 
        ...
        # High confidence threshold for early stopping
        self.high_confidence_threshold = 0.5  # Stop searching if match > 0.5
        ...
        print(f"  → FAISS acceleration: {'ENABLED' if FAISS_AVAILABLE and self.faiss_index else 'DISABLED'}")
    ...
    def save_database(self):
        try:
            ...
            # Rebuild FAISS index when database changes
            self._build_faiss_index()
        except Exception as e:
    ...
    def _build_faiss_index(self):
        ...
        if len(self.known_faces['embeddings']) == 0:
            self.faiss_index = None
            return
        
        try:
            # Stack embeddings into matrix
            embeddings = np.vstack(self.known_faces['embeddings']).astype('float32')
            
            # Normalize for cosine similarity (IP on normalized = cosine)
            faiss.normalize_L2(embeddings)
            
            # Create index - Inner Product for cosine similarity
            dim = embeddings.shape[1]  # 128 for SFace
            self.faiss_index = faiss.IndexFlatIP(dim)
            self.faiss_index.add(embeddings)
            
            print(f"  → FAISS index built: {embeddings.shape[0]} embeddings")
        except Exception as e:
    ...
    def recognize_face(self, frame, bbox):
        """
        OPTIMIZED: Uses FAISS for fast similarity search with early stopping.
        ...
        try:
            ...
            # === FAISS-based search with early stopping ===
            if FAISS_AVAILABLE and self.faiss_index is not None:
                return self._faiss_search(query_embedding)
            else:
                return self._linear_search(query_embedding)
            
        except Exception as e:
            return "Unknown", 0.0
    
    def _faiss_search(self, query_embedding):
        """
        Fast FAISS-based similarity search with early stopping.
        ...
        # Search top-k candidates (k=min(5, db_size))
        k = min(5, len(self.known_faces['names']))
        ...
        for i, idx in enumerate(indices[0]):
            ...
            # Early stopping: if we found a high-confidence match, stop immediately
            if score >= self.high_confidence_threshold:
                return self.known_faces['names'][idx], score
            
            # Track best match above threshold
            if score >= self.threshold and score > best_score:
                best_score = score
                best_match = self.known_faces['names'][idx]
        
        return best_match, best_score
    
    def _linear_search(self, query_embedding):
        """
        Fallback linear search when FAISS is unavailable.
        Also supports early stopping on high-confidence matches.
        """
        best_match = "Unknown"
        best_score = 0.0
